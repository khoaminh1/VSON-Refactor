/* ============================================================
   VSON Advisory — minimal Handlebars build
...
// Handlebars `unless` is built-in, but `@first`/`@last` inside `{{#each}}` work natively.
...
for (const file of fs.readdirSync(PAGES_DIR)) {
  ...
  // Spread pageData keys (meta, hero, value_chain, …) into root context so partials
  // can reach them via inheritance — Handlebars hash partial parameters with dotted
  // RHS (like `hero=page.hero`) do not bind reliably. site/nav/footer listed LAST
  // so they always win over any page-level key collision.
